...
-(UIImage*)captureView{
    ...
    if ([self isKindOfClass:[UIScrollView class]]) {//类判断
...
-(void)drawRectangle:(CGRect)rect
{
    ...
    CGContextDrawPath(context,kCGPathFillStroke);//填充样式
...
{
#warning NSAssert  是苹果自带的一个函数  是 用于调试用的   （前者是条件,后者是当不满足前者时,抛出的异常处理信息）
    /**
     *  <#Description#>
     *
     *  @param <#condition#> <#<#condition#> description#>   condition  是一个条件
     *  @param <#desc        <#<#desc description#>       desc   这个是抛出的异常(当 condition中的条件不满足时,就抛出 desc 自定义的异常提示信息)
     *  @param ...#>         <#...#> description#>
     *
     *  @return <#return value description#>
     */
    //NSAssert(需要满足的条件, 如没有满足条件时的异常处理信息);
    ...
    for(int i = 1;i < pointArray.count;i++)
    {
        ...
        NSValue *pointValue = [pointArray objectAtIndex:i];//将值添加到 pointArray 数组中去
...
{
    NSAssert(pointArray.count>=2,@"数组长度必须大于等于2");//调试的函数(前者是条件,后者是异常处理信息)
    NSAssert([[[pointArray objectAtIndex:0] class] isSubclassOfClass:[NSValue class]], @"数组成员必须是CGPoint组成的NSValue");
