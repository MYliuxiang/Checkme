...
-(id)initWithFrame:(CGRect)frame withUIView:(NSMutableArray*)uiViewArr{
    ...
    NSUInteger pageCount=[uiViewArr count];
    [self setContentSize:CGSizeMake(self.frame.size.width*pageCount, self.frame.size.height)];
//    UIView* defaultUIView = [uiViewArr objectAtIndex:0];//joe add
    CGRect bounds=CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
    for (int i=0; i<pageCount; i++) {
        bounds.origin.x=bounds.size.width*i;
        UIView *uiView=[uiViewArr objectAtIndex:i];
